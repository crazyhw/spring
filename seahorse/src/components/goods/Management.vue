<template>
  <div>
    ...
    <el-table :data="tableData">
      <el-table-column  label="仓库编号" width="140">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.num }}</span>
        </template>
      </el-table-column>
      <el-table-column  label="仓库名称" width="140">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column  label="仓库容量" width="140">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.number }}</span>
        </template>
      </el-table-column>
      <el-table-column  label="仓库地址" width="140">
        <template slot-scope="scope">
      ...
      <el-table-column  label="操作" >
        <template slot-scope="scope">
          ...
          <el-dialog title="修改仓库信息" :visible.sync="dialogUpdateVisible">
            <el-form :model="Form">
              <el-form-item label="仓库编号" :label-width="formLabelWidth">
                <el-input v-model="Form.num" autocomplete="off"></el-input>
              </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      postForm() {
        const url = 'management/addManagement/';
      ...
      handleDelete(index, row) {
        ...
        this.$axios
          .delete('management/deleteManagement/' + expenditureId)
          .then(res => {
            console.log(res);
