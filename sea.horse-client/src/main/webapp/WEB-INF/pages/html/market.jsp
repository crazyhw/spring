...
<div class="container-fluid">
    ...
    <div class="row row-offcanvas row-offcanvas-left">
        ...
        <div class="col-sm-9 col-md-10 main">
            <h2 class="sub-header">商场信息</h2>
            <div class="row">
                <div class="col-lg-2">
                    <div class="input-group">
                        <input type="text" class="form-control" placeholder="请输入商场名称" id="write">
                        <span class="input-group-btn">
                            <button class="btn btn-default" type="button" onclick="searchMarket()">查询</button>
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-dialog" style="width:300px">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">添加商场信息</h4>
                        </div>
                        <div class="modal-body" id="myModalBody">
                            <div class="input-group my-center">
                                <span class="input-group-addon">商场名称</span>
                                <input type="text" class="form-control" placeholder="请输入商场名称" id="name">
                            </div>
                            <br/>
                            <div class="input-group my-center">
                                <span class="input-group-addon">商场地址</span>
                                <input type="text" class="form-control" placeholder="请输入商场地址" id="address">
                            </div>
                            <br/>
                        </div>
                        <div class="modal-footer">
                            <button id="modal-submit" type="button" class="btn btn-primary" onclick="addMarket()">确认</button>
...
<script type="application/javascript">
    ...
    function getdata() {
        ...
        $.ajax({
            url:"http://localhost:8080/seahorse/market/all",
            type:"get",
            dataType:"JSON",
            async:false,
    ...
    $(document).on("click", ".delete", function () {
        ...
        if(confirm("是否删除？")){
            $.ajax({
                type:"post",
                url:"http://localhost:8080/seahorse/market/deleteMarket",
                data:id ,
                dataType:"JSON",
                async:false,
    ...
    function addMarket(){
        var market = {};
        market.name=$('#name').val();
        market.address=$('#address').val();
        $.ajax({
            type:"post",
            url:"http://localhost:8080/seahorse/market/addMarket",
    ...
    $(document).on("click", ".update", function () {
        ...
        $("#modal-submit").attr("onclick", "updateMarket("+id+")");
    });
    function clearModal() {
    ...
    function updateMarket(id) {
        ...
        $.ajax({
            type:"post",
            url:"http://localhost:8080/seahorse/market/updateMarket",
            data: JSON.stringify(market),
            dataType:"JSON",
            async:false,
            contentType:"application/json;charset=utf-8",
            success:function (data) {
                if(data==true) {
                    $("#myModal").modal('hide');
                    $("#modal-submit").attr("onclick", "addMarket()");
                    clearModal();
                    getdata();
                }else{
                    alert("修改失败！");
    ...
    function searchMarket() {
        ...
        $.ajax({
            type:"post",
            url:"http://localhost:8080/seahorse/market/searchMarket",
            data:name ,
            dataType:"JSON",
            async:false,
            contentType:"application/json;charset=utf-8",
